...
| `x` | number | ✓ | ✓ | 顯示座標（1~42，1 位小數）。**全 340 筆＝遊戲原生 Level sheet**（`Adventure.Level`→`tc_Level` X/Z ＋ `tc_Map` SizeFactor 換算；2026-07-18 起取代 cycleapple/babelin） |
| `y` | number | ✓ | ✓ | 同上 |
| `z` | number\|null | — | 選填 | HW–DT 高度（babelin 人工量測）；ARR 省略——遊戲 Level.Y 無穩定 world→顯示 Z 換算（同 z 對應迥異 Level.Y、全域擬合 129/181 差 >0.5），無社群 z 可校準 |
...
- **座標 X/Y（2026-07-18 起）改採遊戲原生 `tc_Level`＋`tc_Map`**（`build_data.py level_coords`，`_to_map` world→map 公式）：上述 cycleapple/babelin 的 x,y 已停用；cycleapple 僅留 weather/time(ARR)、babelin 僅留 zone/z(HW–DT)。對標驗證：HW–DT vs babelin 差 mean 0.08、對 Google Sheet(wikiwiki.jp) 18/20 差 <0.1。
