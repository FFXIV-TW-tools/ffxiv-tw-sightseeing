...
正典：`~/.claude/process/DEVLOOP.md`；本 repo 工件：`CHANGELOG.md`、`docs/BACKLOG.md`；按需建 spec 於 `docs/specs/`（`-design.md`＋front-matter）、plan 於 `docs/plans/`（v1.10 工件位置＝契約）。摘要（對齊 DEVLOOP v1.18；正典不可得時以此為準）：
